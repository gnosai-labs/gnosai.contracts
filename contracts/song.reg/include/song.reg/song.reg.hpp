...
struct [[eosio::table("songs"), eosio::contract("song.reg")]] song_info {
    ...
    EOSLIB_SERIALIZE(song_info, (nid)(creator)(token_url)(music_hash)(music_url)(create_at)(update_at))
};
...
public:
    ...
    [[eosio::action]]
    void regsong(eosio::name creator, uint64_t nid, std::string token_url, std::string music_hash, std::string music_url);

    [[eosio::on_notify("cisum.token::transfer")]]
    void ontransfer(eosio::name from, eosio::name to, eosio::asset quantity, std::string memo);

private:
    void create_song_nft(uint64_t nft_id, uint32_t issue_count, const song_reg_global& gstate);
    void buy_song_nft(eosio::name buyer, uint64_t nft_id, uint32_t buy_count);
    uint32_t parse_songcreate_memo(const std::string& memo, uint64_t& nft_id) const;
